...
on your root fs with \"-r / -a init=/bin/bash\"
 ...
 -t, --tap
 Use this bringed TAP, set up via a live interface as:
...
GETOPT=`getopt -o a:d:e:Dhbr:sS:vt: --long append:,disk:ext:,no-gdb,help,boot,root:,sdl,smp:,vnc,tap: \
 -n "$(basename \"$0\")" -- "$@"`
...
QEMU_OPTIONS="$QEMU_OPTIONS -smp $SMP -m size=4000"
if [ "$TAP" ]; then
    QEMU_OPTIONS="$QEMU_OPTIONS -device e1000,netdev=network0,mac=52:55:00:d1:55:01 -netdev tap,id=network0,ifname=tap0,script=no,downscript=no"
fi
...
GDB_PID=
if [ "$USE_GDB" -a "$DISPLAY" -a -x "$(which xterm)" -a -e "$(which cgdb)" ]; then
